...
class RayToWavefrontReconstructor:
    ...
    def _compute_amplitude_phase_jacobian(
        ...
        优化：如果提供了 grid_metadata（来自笛卡尔网格采样），则使用快速有限差分计算，
        避免昂贵的 RBF 插值。
        
        参数:
            grid_metadata: 包含 'shape', 'sampling_indices', 'grid_dx' 的字典
        """
        ...
        if grid_metadata is not None:
            try:
                # 1. 重建完整输出网格
                # grid_metadata['shape'] 是完整网格形状 (ny, nx)
                full_shape = grid_metadata['shape']
                stride_mm = grid_metadata['grid_dx']
                
                # ray_x_out 是扁平化的数组，包含无效点（如果是在采样时未过滤）
                # 或者我们需要根据 grid_metadata 恢复结构
                # 假设 ray_x_out 对应的是全网格的扁平化数据（如果 hybrid_element_propagator 没过滤）
                # 检查数据长度是否匹配全网格
                if len(ray_x_out) == np.prod(full_shape):
                    x_out_grid = ray_x_out.reshape(full_shape)
                    y_out_grid = ray_y_out.reshape(full_shape)
                else:
                    # 如果这不可能发生（因为我们在 hybrid 中修改了不过滤），则回退
                    raise ValueError(f"Grid metadata provided but ray count {len(ray_x_out)} != {np.prod(full_shape)}")
                
                # 2. 使用 numpy gradient 计算偏导数
                # 注意 gradient 返回 [d/dy, d/dx]
                # x 方向导数 (axis 1)
                dx_out_dy_in_grid, dx_out_dx_in_grid = np.gradient(x_out_grid, stride_mm)
                # y 方向导数 (axis 1)
                dy_out_dy_in_grid, dy_out_dx_in_grid = np.gradient(y_out_grid, stride_mm)
                
                # 3. 计算雅可比行列式
                # J = dx/dx * dy/dy - dx/dy * dy/dx
                jacobian_grid = np.abs(
                    dx_out_dx_in_grid * dy_out_dy_in_grid - 
                    dx_out_dy_in_grid * dy_out_dx_in_grid
                )
                
                # 4. 提取有效光线的雅可比值
                jacobian_det = jacobian_grid.flatten()[valid_mask]
                ...
                # 构建结果数组
                amplitude = np.zeros(len(ray_x_in)) # 保持原始长度
                amplitude[valid_mask] = amplitude_valid
                
                # 相位
                valid_opd = opd_waves[valid_mask]
                phase = np.zeros(len(ray_x_in))
                phase[valid_mask] = 2 * np.pi * valid_opd
                
                # print(f"[DEBUG] Fast Jacobian used. Speedup achieved.")
                return amplitude, phase
                
            except Exception as e:
                print(f"[WARNING] Fast Jacobian failed: {e}. Falling back to RBF.")
                # Fallthrough to RBF
    ...
    def reconstruct_amplitude_phase(
        ...
        参数:
            ...
            grid_metadata: 采样网格元数据，用于快速雅可比计算（可选）
