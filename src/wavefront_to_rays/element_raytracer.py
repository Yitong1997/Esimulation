...
class SurfaceDefinition:
    ...
    radius_x: float = field(default_factory=lambda: np.inf)  # X 方向曲率半径 (仅用于 Biconic)
    thickness: float = 0.0
    material: str = 'mirror'
    semi_aperture: Optional[float] = None
    conic: float = 0.0  # 圆锥常数，默认 0.0（球面）
    conic_x: float = 0.0  # X 方向圆锥常数 (仅用于 Biconic)
    ...
    def __post_init__(self) -> None:
        """初始化后验证参数有效性"""
        # 验证表面类型
        valid_types = ('mirror', 'refract', 'biconic')
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'radius_x': float(self.radius_x) if np.isfinite(self.radius_x) else 'inf',
            'thickness': self.thickness,
            'material': self.material,
            'semi_aperture': self.semi_aperture,
            'conic': self.conic,
            'conic_x': self.conic_x,
            'tilt_x': self.tilt_x,
            'tilt_y': self.tilt_y,
...
class ElementRaytracer:
    ...
    def __init__(
        ...
        参数:
            ...
            exit_chief_direction: 出射主光线方向向量 (L, M, N)，可选
                                 ...
                                 必须是归一化的方向余弦
            incident_material: 入射介质名称 (默认 'air')
            debug: 是否开启调试模式
        ...
        self.incident_material = incident_material
        self.debug = debug
    ...
    def _create_optic_base(self) -> None:
        ...
        # =====================================================================
        # 添加物面（index=0）
        # 修正：使用 incident_material 设置物方介质
        optic.add_surface(index=0, thickness=0.0, material=self.incident_material)
        
        # 添加虚拟光阑面（在入射面位置）
        # ⚠️ 这里设为 STOP，位于 (0,0,0)
        # 用 thickness=0.0 避免 numpy.inf 导致的可选坐标更新问题
        # 后续表面使用绝对坐标 (x,y,z) 定位，因此此处的 thickness 不影响几何位置
        # 修正：使用 incident_material 设置光阑前后的介质
        optic.add_surface(index=1, thickness=0.0, is_stop=True, material=self.incident_material)
        ...
        for i, surface_def in enumerate(self.surfaces):
            ...
            if surface_def.orientation is not None:
                ...
                if N < 0:
                    ...
                    # 同时必须修正旋转角度，使法向反转
                    # 
                    # 当 Nz < 0 时，表面局部坐标系 Z 轴背对入射光。
                    # 为了在 optiland 中正确建模（optiland 默认光线沿局部 +Z 传播），
                    # 我们需要将局部坐标系旋转 180 度，使 Z 轴指向"前方" (+Z global)。
                    # 
                    # 几何变换推导 (将法向量 n 变为 -n):
                    # 原始: L=sin(ry)cos(rx), M=-sin(rx), N=cos(ry)cos(rx)
                    # 目标: L'=-L, M'=-M, N'=-N
                    #
                    # 1. M' = -M => -sin(rx') = -(-sin(rx)) = sin(rx)
                    #    => sin(rx') = -sin(rx) => rx' = -rx
                    # 
                    # 2. L'=-L, N'=-N => ry' = arctan2(-L, -N) = ry + pi
                    
                    tilt_x_rad = -tilt_x_rad
                    tilt_y_rad += np.pi
                    
                    if getattr(self, 'debug', False):
                        print(f"[DEBUG ElementRaytracer] Surface {surface_index}: Normal facing away (Nz={N:.4f}).")
                        print(f"  Action: Flip Radius & Rotate 180 deg")
                        print(f"  New Radius: {radius}, New RadiusX: {radius_x}")
                        print(f"  New Tilt: rx={np.degrees(tilt_x_rad):.2f}°, ry={np.degrees(tilt_y_rad):.2f}°")
            ...
            # 准备额外参数
            extra_kwargs = {}
            if surface_def.surface_type == 'biconic':
                extra_kwargs['surface_type'] = 'biconic'
                extra_kwargs['radius_x'] = radius_x
                extra_kwargs['radius_y'] = radius # 使用处理过符号的 Y 半径
                extra_kwargs['conic_x'] = surface_def.conic_x
                # conic_y 由 conic 参数传递
                # optiland 可能需要 conic_y 键
                extra_kwargs['conic_y'] = surface_def.conic
    ...
    def _trace_with_signed_opd(self, rays: RealRays, skip: int = 1) -> None:
        ...
        # ⚠️ 关键修复：坐标系归一化
        # =====================================================================
        # Optiland 的 trace() 方法会将光线留在入射面局部坐标系中
