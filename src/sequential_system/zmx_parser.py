...
@dataclass
class CoordinateBreakData:
    """表面坐标断点数据 (SCBD)
    
    用于存储 SCBD 指令定义的坐标变换参数。
    SCBD 指令格式: SCBD <type> <order> <reserved> <dx> <dy> <tx> <ty> <tz>
    """
    order: int                  # 0: 先平移后旋转, 1: 先旋转后平移
    decenter_x: float           # X 方向偏心 (mm)
    decenter_y: float           # Y 方向偏心 (mm)
    tilt_x_deg: float           # X 轴旋转 (度)
    tilt_y_deg: float           # Y 轴旋转 (度)
    tilt_z_deg: float           # Z 轴旋转 (度)
...
class ZmxSurfaceData:
    ...
    # SCBD 数据 (Surface Coordinate Break Data)
    scbd_pre: Optional[CoordinateBreakData] = None   # 面之前的坐标断点 (SCBD Type 1)
    scbd_post: Optional[CoordinateBreakData] = None  # 面之后的坐标断点 (SCBD Type 2)
...
class ZmxParser:
    ...
    def _parse_parm(self, data: List[str]) -> None:
        ...
        # 只处理坐标断点和近轴面形类型的表面
        if self._current_surface.surface_type not in ('coordinate_break', 'paraxial', 'biconic'):
            return
        ...
        elif self._current_surface.surface_type == 'biconic':
            # BICONIC 参数映射
            if param_index == 1:
                # PARM 1 → Radius X (mm)
                if abs(param_value) < 1e-14:
                    self._current_surface.radius_x = np.inf
                else:
    ...
    def _parse_scbd(self, data: List[str]) -> None:
        """解析 SCBD 操作符（表面坐标断点数据）
        
        解析 SCBD 指令，设置表面前或后的坐标变换参数。
        
        参数:
            data: SCBD 操作符后的数据列表 (至少 8 个参数)
                - data[0]: Type (1=Pre, 2=Post)
                - data[1]: Order (0=Trans->Rot, 1=Rot->Trans)
                - data[2]: Reserved (默认 0，忽略)
                - data[3]: Decenter X
                - data[4]: Decenter Y
                - data[5]: Tilt X
                - data[6]: Tilt Y
        ...
        说明:
            SCBD 是 "Surface Coordinate Break Data" 的缩写。
            它允许在不插入额外 COORDBRK 面的情况下定义坐标变换。
            
            Type 1 (Pre): 在表面形状定义之前应用变换。
            Type 2 (Post): 在表面形状定义之后（但在厚度传播之前）应用变换。
        """
        if len(data) < 8 or self._current_surface is None:
            return
            
        try:
            scbd_type = int(data[0])
            order = int(data[1])
            # data[2] is reserved
            decenter_x = float(data[3])
            decenter_y = float(data[4])
            tilt_x = float(data[5])
            tilt_y = float(data[6])
            tilt_z = float(data[7])
            
            scbd_data = CoordinateBreakData(
                order=order,
            ...
            if scbd_type == 1:
                self._current_surface.scbd_pre = scbd_data
            elif scbd_type == 2:
                self._current_surface.scbd_post = scbd_data
                
        except ValueError:
            raise ZmxParseError(
                f"无效的 SCBD 参数: {' '.join(data)}",
                line_number=self._current_line_number,
                line_content=self._current_line_content
            )
