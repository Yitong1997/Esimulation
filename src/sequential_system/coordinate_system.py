...
if TYPE_CHECKING:
    from sequential_system.zmx_parser import ZmxDataModel, ZmxSurfaceData, CoordinateBreakData
...
class SurfaceTraversalAlgorithm:
    ...
    **Validates: Requirements 5.1, 5.2, 5.3, 5.4, 5.5, 5.6**
    ...
    def _process_optical_surface(self, surface: 'ZmxSurfaceData') -> None:
        ...
        流程:
        1. 如果有 Pre-SCBD，应用坐标变换
        2. 生成全局坐标定义 (基于 Pre-SCBD 变换后的坐标系)
        3. 如果有 Post-SCBD，应用坐标变换
        4. 更新坐标系（沿 Z 轴前进厚度）
        
        参数:
            surface: ZmxSurfaceData 对象
        
        **Validates: Requirements 5.4, 9.1, 9.2, SCBD Support**
        """
        # 1. 应用 Pre-SCBD (如果存在)
        if surface.scbd_pre:
            self._apply_scbd(surface.scbd_pre)

        # 2. 创建全局坐标表面定义 (使用当前的坐标系状态)
        global_surface = self._create_global_surface(surface)
        self._global_surfaces.append(global_surface)
        
        # 3. 应用 Post-SCBD (如果存在)
        if surface.scbd_post:
            self._apply_scbd(surface.scbd_post)

        # 4. 更新坐标系：沿当前 Z 轴前进厚度
        # 주의: 即使 Post-SCBD 改变了方向，厚度也是沿着变换后新的 Z 轴传播
    ...
    def _apply_scbd(self, scbd: 'CoordinateBreakData') -> None:
        """应用 SCBD 变换到当前坐标系"""
        tilt_x_rad = np.deg2rad(scbd.tilt_x_deg)
        tilt_y_rad = np.deg2rad(scbd.tilt_y_deg)
        tilt_z_rad = np.deg2rad(scbd.tilt_z_deg)
        
        # SCBD 自身没有 "厚度" 概念，它只是表面位置的修正
        # 所以 thickness 传 0
        self._current_cs = CoordinateBreakProcessor.process(
            current_cs=self._current_cs,
            decenter_x=scbd.decenter_x,
            decenter_y=scbd.decenter_y,
            tilt_x_rad=tilt_x_rad,
            tilt_y_rad=tilt_y_rad,
            tilt_z_rad=tilt_z_rad,
            order=scbd.order,
