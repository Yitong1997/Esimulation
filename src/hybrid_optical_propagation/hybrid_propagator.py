    ...
    def _trace_chief_ray_optiland(self) -> List[Dict[str, np.ndarray]]:
        ...
        for i in range(len(self._optical_system)):
            ...
            if len(opt_surface.x) > 0:
                ...
                user_surf = self._optical_system[i]
                print(f"[DEBUG_CHIEF] Map User Surf {i} -> Optiland Surf {opt_surface_index} ({type(opt_surface).__name__})")
                print(f"    Details: Type={user_surf.surface_type}, R={user_surf.radius}, Conic={user_surf.conic}, Mat={user_surf.material}")
                if user_surf.surface_type == 'biconic':
                     print(f"             Biconic: Rx={user_surf.radius_x}, Kx={user_surf.conic_x}")
        ...
        for i, surface in enumerate(self._optical_system):
            ...
            if interaction_type in ['free_space', 'coordinate_break']:
                ...
                current_state = self._hybrid_element_propagator.propagate(
                    ...
                    exit_axis,
                    target_surface_index=i,
                    incident_material_name=current_material_name,
                )
                
            # 更新当前介质
            # 只有折射面会改变介质
            if not surface.is_mirror and not interaction_type == 'coordinate_break':
                 # 如果是透镜或折射面，出射介质就是表面的材料
                 # 注意：optiland 的材料定义通常指"该表面之后的介质"
                 # 因此，穿过表面 i 后，介质变为 surface[i].material
                 # 除非是空气隙?
                 # 在 Zemax 中，Surface i defines material AFTER surface i.
                 # So yes, update current_material.
                 current_material_name = surface.material
