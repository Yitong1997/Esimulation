...
class StateConverter:
    ...
    **Validates: Requirements 5.1-5.5, 9.1-9.5**
    ...
    def _sync_gaussian_params(
        ...
        修正：由于 PROPER 在真空中运行，我们需要将介质中的物理距离 z, z_R
        转换为"真空等效距离" z/n, z_R/n。
        - z_w0 (vac) = z (vac) + waist_position_mm / n
        
        转换关系：
        - 当前位置相对于束腰的距离 = -waist_position_mm
        - z_w0 = z - (-waist_position_mm/n) = z + waist_position_mm/n
        """
        import proper
        ...
        # 修正：z_R (vac) = z_R (med) / n
        wfo.z_Rayleigh = (pilot_beam_params.rayleigh_length_mm / current_n) * 1e-3
        # 正确的转换：z_w0 = z + waist_position_mm / n
        wfo.z_w0 = wfo.z + (pilot_beam_params.waist_position_mm / current_n) * 1e-3
    ...
    def check_gaussian_params_sync(
        ...
        **Validates: Requirements 5.5, 9.5**
        ...
        # 比较时需要将 scaled PROPER z_R 还原回物理 z_R (z_R_phys = z_R_vac * n)
        # 或者将 pilot z_R 转换为 vacuum z_R
        # 这里统一转为 vacuum z_R 比较
        z_R_pilot_vac_mm = pilot_beam_params.rayleigh_length_mm / current_n
        
        w0_error = abs(w0_proper_mm - w0_pilot_mm) / w0_pilot_mm if w0_pilot_mm > 0 else 0
        z_R_error = abs(z_R_proper_mm - z_R_pilot_vac_mm) / z_R_pilot_vac_mm if z_R_pilot_vac_mm > 0 else 0
