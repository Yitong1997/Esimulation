    ...
    **Validates: Requirements 6.1-6.9, 7.1-7.7**
        ...
        **Validates: Requirements 6.1, 7.7**
        ...
        if self._method == "local_raytracing":
            return self._propagate_local_raytracing(
                state, surface, entrance_axis, exit_axis, target_surface_index, incident_material_name
            )
        else:
            #报错
    ...
    def _propagate_local_raytracing(
        ...
        **Validates: Requirements 6.2-6.9**
        ...
        # 1. 从振幅/相位采样光线
        # 注意：相位是非折叠实数，不需要解包裹
        input_rays, grid_metadata = self._sample_rays_from_wavefront(
            state.amplitude,
            state.phase,
            state.grid_sampling,
            entrance_axis,
        ...
        #为我打印debug信息
        print(f"\n[DEBUG] Surface {target_surface_index}: Type={surface.surface_type}, Radius={surface.radius}, Conic={surface.conic}, Material={surface.material}")
        print(f"[DEBUG] Input Rays Analysis:")
        ...
        current_n = new_pilot_params.current_refractive_index
        k = 2 * np.pi * current_n / wavelength_mm
        ...
        if np.isinf(R_out): 
            ...
            # 介质中的 OPD 需要乘以折射率 n
            pilot_opd_mm = current_n * r_sq_out / (2 * R_out)
    ...
    def _propagate_pure_diffraction(
        ...
        **Validates: Requirements 7.1-7.6**
        ...
        if total_rotation_deg > 179.9:
            ...
            k = 2 * np.pi / wavelength_mm
            if surface.is_mirror:
                # 修正：Mirror in medium n: 2*k*n*sag
                current_n = state.pilot_beam_params.current_refractive_index
                phase_delay = 2 * k * current_n * sag
            else:
                n1 = 1.0
                n2 = self._get_refractive_index(surface.material)
                phase_delay = k * sag * (n2 - n1)
    ...
    def _sample_rays_from_wavefront(
        ...
        """
        from optiland.rays import RealRays  # Opt test
        # Remove RegularGridInterpolator as we don't need it for grid-aligned sampling
        
        # 1. 准备采样用的相位
        # 分离 Pilot Beam（如果存在），只对残差相位与振幅进行 Padding
        ...
        # --- OVERSAMPLING & PADDING START ---
        # 引入过采样以解决光束收缩导致的边缘重建异常问题
        coverage_factor = 1.25
        n_original = grid_sampling.grid_size
        n_padded = int(n_original * coverage_factor)
        pad_width = (n_padded - n_original) // 2
        
        # 如果 pad_width > 0，执行填充
        if pad_width > 0:
             # Amplitude: 补 0 (物理孔径外无能量)
             amplitude_padded = np.pad(amplitude, pad_width, mode='constant', constant_values=0.0)
             
             # Phase: 使用 edge 填充 (延续边界值，避免梯度剧变)
             sampling_phase_padded = np.pad(sampling_phase, pad_width, mode='edge')
             
             # 更新尺寸参数
             n = n_padded
             # 实际上我们不需要更新 physical_size_mm，因为 grid_sampling 对象没变
             # 我们只是在生成 coords 时使用更宽的范围
        else:
             amplitude_padded = amplitude
             sampling_phase_padded = sampling_phase
             n = n_original
        # --- OVERSAMPLING & PADDING END ---
        
        #绘制采样相位
        # self._plot_debug_phase(sampling_phase, 0,is_pltshow=True)

        # 2. 生成网格坐标 (Correct Method)
        # n 已经是更新后的尺寸 (n_padded)
        dx = grid_sampling.sampling_mm
        dy = grid_sampling.sampling_mm
        
        # 使用 arange 生成坐标，确保 0.0 在像素中心
        # 注意：这里的范围通过 n 自动扩大了，dx 保持不变
        coords = (np.arange(n) - n // 2) * dx
        
        # 3. 笛卡尔网格采样
        # 保持原始的采样密度 (stride 基于原始尺寸计算)
        # 这样会增加总光线数，但保持了空间分辨率
        
        num_rays_target = self._num_rays
        stride = max(1, int(n_original / np.sqrt(num_rays_target))) # Use n_original for stride
        ...
        # 确定最大可能的偏移量
        max_offset = max(center_idx, n - 1 - center_idx)
        
        # 生成偏移序列
        offsets = np.arange(0, max_offset + 1, stride)
        ...
        for off in offsets:
             ...
             # 右侧点
             r_idx = center_idx + off
             if r_idx < n:
                 valid_indices.append(r_idx)
             
             # 左侧点
             l_idx = center_idx - off
             if l_idx >= 0:
                 valid_indices.append(l_idx)
                 
        sample_indices = np.sort(np.unique(valid_indices))
        ...
        # 提取数据 (从 padded 数组中提取)
        sampled_phase = sampling_phase_padded[iy_flat, ix_flat]
        sampled_amp = amplitude_padded[iy_flat, ix_flat]
        ...
        # 先计算全分辨率的梯度，然后提取
        grad_y, grad_x = np.gradient(sampling_phase_padded, dy, dx)
        
        wavelength_mm = self._wavelength_um * 1e-3
        current_n = pilot_beam_params.current_refractive_index if pilot_beam_params is not None else 1.0
        # 使用介质折射率修正波数
        k = 2 * np.pi * current_n / wavelength_mm
        ...
        full_grid_shape = ix_grid.shape
        
        from wavefront_to_rays.element_raytracer import compute_rotation_matrix, transform_rays_to_global

        # Construct LOCAL rays
        # Handle TIR / Evanescent roughly by masking N=0
        sin_sq = L_final**2 + M_final**2
        N_local = np.sqrt(np.maximum(1.0 - sin_sq, 0))
        ...
        # 提取完整相位（用于计算初始 OPD）
        # sampled_phase 已经在前面提取了 (line 1097 in original view, roughly)
        # 对应 original: sampled_full_phase = phase[iy_flat, ix_flat] which was used for filtering
        # Here we just use sampled_phase which is already the full grid data
        initial_phase = sampled_phase
        initial_opd_mm = initial_phase * wavelength_mm / (2 * np.pi)
        output_rays.opd = initial_opd_mm
        output_rays.i = sampled_amp**2
        
        grid_metadata = {
            'shape': full_grid_shape,
            'sampling_indices': sample_indices,
            'grid_dx': dx * stride
        }
        
        return output_rays, grid_metadata
    ...
    def _update_pilot_beam(
        ...
        - 圆锥曲面（含抛物面）：使用局部切向曲率半径更新
          kappa_t = 1 / (R * [1 - k * (d/R)^2]^(3/2)),  R_t = 1 / kappa_t
        - 折射面：使用折射面的 ABCD 矩阵
        ...
        **Validates: Requirements 6.7**
        ...
        if np.isinf(R):
            ...
            # 切向曲率半径（任意 conic）
            # kappa_t = 1 / (R * [1 - k * (d/R)^2]^(3/2))
            ratio = d / R
            factor = 1.0 - conic * (ratio**2)
            if factor <= 0.0:
                # 超出有效区域时，避免产生复数/NaN
                effective_R_surf = np.inf
            else:
                effective_R_surf = R * (factor**1.5)
        ...
        if surface.is_mirror:
            ...
            return pilot_params.apply_refraction(effective_R_abcd, n1, n2)
