    ...
    **Validates: Requirements 8.1, 8.4**
    ...
    def propagate(
        ...
        **Validates: Requirements 8.2, 8.3**
        ...
        local_rays = self._sample_rays_from_wavefront(
            ...
            state.phase,
            state.grid_sampling,
            current_refractive_index=state.pilot_beam_params.current_refractive_index,
        )
    ...
    def _sample_rays_from_wavefront(
        ...
        **Validates: Requirements 5.1**
        ...
        current_n = current_refractive_index if current_refractive_index and current_refractive_index > 0 else 1.0
        # WavefrontToRaysSampler 使用真空波数，需将相位按 n 归一化以获得正确方向
        sampling_phase = phase / current_n
    ...
    def _compute_pilot_opd(
        ...
        **Validates: Requirements 7.2**
        ...
        wavelength_mm = self._wavelength_um * 1e-3
        current_n = pilot_params.current_refractive_index
        
        if np.isinf(R):
            pilot_opd_mm = np.zeros_like(r_sq)
        else:
            # 介质中的 OPD 需要乘以折射率 n
            pilot_opd_mm = current_n * r_sq / (2 * R)
    ...
    def _update_pilot_beam(
        ...
        pilot_params: PilotBeamParams,
        surface: "GlobalSurfaceDefinition",
        entrance_axis: "OpticalAxisState" | None = None,
    ) -> PilotBeamParams:
        """更新 Pilot Beam 参数
        ...
        **Validates: Requirements 9.1-9.4**
        ...
        if entrance_axis is not None:
            entrance_dir = np.asarray(entrance_axis.direction.to_array(), dtype=float)
            surface_normal = np.asarray(surface.orientation[:, 2], dtype=float)
