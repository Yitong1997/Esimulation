    ...
    **Validates: Requirements 8.1, 8.2, 8.3, 8.4, 8.5, 8.6**
    ...
    def from_q_parameter(
        ...
        **Validates: Requirements 8.1, 8.2**
        ...
        # 修正：包含折射率 n: w^2 = -lambda_0 / (pi * n * imag(1/q))
        w_sq = -wavelength_mm / (np.pi * current_refractive_index * imag_part)
        w = np.sqrt(w_sq) if w_sq > 0 else 0.0
        
        # 瑞利长度和束腰半径
        z_R = np.imag(q)
        # 修正：包含折射率 n: w0^2 = z_R * lambda_0 / (pi * n)
        w0 = np.sqrt(wavelength_mm * z_R / (np.pi * current_refractive_index)) if z_R > 0 else w
    ...
    def propagate(self, distance_mm: float) -> "PilotBeamParams":
        ...
        **Validates: Requirements 8.3**
        ...
        return PilotBeamParams.from_q_parameter(q_new, self.wavelength_um, self.current_refractive_index)
    ...
    def compute_phase_at_radius(self, r_mm: float) -> float:
        ...
        **Validates: Requirements 8.5, 8.6**
        ...
        # 修正：使用当前折射率计算波数 k = 2 * pi * n / lambda_0
