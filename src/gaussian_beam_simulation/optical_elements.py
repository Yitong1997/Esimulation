...
@dataclass
class BiconicMirror(OpticalElement):
    """双锥面反射镜 (Biconic Mirror)
    
    表面由两个正交方向的曲率半径和圆锥常数定义。
    
    参数:
        thickness: 到下一元件的间距（mm）
        semi_aperture: 半口径（mm）
        radius_x: X 方向曲率半径 (mm)
        radius_y: Y 方向曲率半径 (mm)
        conic_x: X 方向圆锥常数
        conic_y: Y 方向圆锥常数
        tilt_x, tilt_y: 旋转角度（rad）
        decenter_x, decenter_y: 偏心（mm）
    ...
    def __post_init__(self) -> None:
        ...
        self._validate_base_parameters()
        # 验证额外参数
        if not isinstance(self.radius_x, (int, float)):
             raise ValueError("radius_x must be numeric")
        if not isinstance(self.radius_y, (int, float)):
             raise ValueError("radius_y must be numeric")
             
    @property
    def focal_length(self) -> float:
        """Y 方向焦距 (近似值)"""
        # 对于双锥面，没有单一焦距。返回 Y 方向焦距作为参考（通常 Y 是切向）
        if np.isinf(self.radius_y):
            return np.inf
        return -self.radius_y / 2.0
        
    @property
    def element_type(self) -> str:
        return "BiconicMirror"
    ...
    def get_surface_definition(self):
        """获取 optiland 表面定义"""
        # 注意：这里需要导入 SurfaceDefinition，但避免循环导入
        # 我们返回字典参数，或者在方法内部导入
        from wavefront_to_rays.element_raytracer import SurfaceDefinition
        
        return SurfaceDefinition(
            surface_type='biconic',
            radius=self.radius_y,  # 将 Y 半径作为标准半径 (Zemax 约定)
            thickness=0.0,
            material='mirror',
            semi_aperture=self.semi_aperture,
            conic=self.conic_y,    # 将 Y 圆锥作为标准圆锥
            radius_x=self.radius_x, # 传递 radius_x
            conic_x=self.conic_x,   # 传递 conic_x
            tilt_x=self.tilt_x,
            tilt_y=self.tilt_y,
        )
