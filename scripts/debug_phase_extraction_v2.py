"""
详细分析从 PROPER 提取相位的每一步 - 版本2

使用更长的传播距离，确保 PROPER 使用 SPHERI 参考面。
同时测试相位折叠的情况。
"""
...
beam_ratio = (2 * w0_m) / beam_diameter_m
...
# 传播距离：超过瑞利距离的 rayleigh_factor 倍
propagation_distance_mm = z_R_mm * proper.rayleigh_factor * 2  # 确保超过阈值
propagation_distance_m = propagation_distance_mm * 1e-3

print(f"\n传播距离: {propagation_distance_mm:.2f} mm")
print(f"传播距离 / 瑞利长度: {propagation_distance_mm / z_R_mm:.2f}")
...
# 检查残差相位是否在 [-π, π] 范围内
if np.min(residual_phase) >= -np.pi - 0.01 and np.max(residual_phase) <= np.pi + 0.01:
...
print(f"  曲率半径差异: {abs(R_theory_mm - R_proper_mm):.2f} mm ({abs(R_theory_mm - R_proper_mm)/R_theory_mm*100:.2f}%)")
...
# ============================================================================
# 步骤 7: 检查 prop_get_phase 是否返回折叠相位
# ============================================================================

print_section("步骤 7: 检查 prop_get_phase 的行为")

# 直接检查 wfarr 的相位
wfarr_centered = proper.prop_shift_center(wfo.wfarr)
wfarr_phase = np.angle(wfarr_centered)

print(f"wfarr 的相位（np.angle）:")
print(f"  最小值: {np.min(wfarr_phase):.6f} rad")
print(f"  最大值: {np.max(wfarr_phase):.6f} rad")
print(f"  范围: {np.max(wfarr_phase) - np.min(wfarr_phase):.6f} rad")

# 与 prop_get_phase 比较
phase_diff_wfarr = residual_phase - wfarr_phase
print(f"\nprop_get_phase 与 np.angle(wfarr) 的差异:")
print(f"  最大差异: {np.max(np.abs(phase_diff_wfarr)):.6f} rad")

# 检查是否有 2π 跳变
print(f"\n检查相位梯度（寻找 2π 跳变）:")
grad_y, grad_x = np.gradient(absolute_phase)
max_grad = np.max(np.abs(grad_y[valid_mask]))
print(f"  Y 方向最大梯度: {max_grad:.6f} rad/pixel")
print(f"  如果 > π，则存在 2π 跳变: {'是' if max_grad > np.pi else '否'}")
...
print_section("步骤 8: 生成可视化图表")

fig, axes = plt.subplots(2, 3, figsize=(15, 10))

# 残差相位
im1 = axes[0, 0].imshow(residual_phase, cmap='RdBu_r', origin='lower')
axes[0, 0].set_title('残差相位 (prop_get_phase)')
plt.colorbar(im1, ax=axes[0, 0], label='rad')

# PROPER 参考相位
im2 = axes[0, 1].imshow(proper_ref_phase, cmap='RdBu_r', origin='lower')
axes[0, 1].set_title('PROPER 参考相位 (解析计算)')
plt.colorbar(im2, ax=axes[0, 1], label='rad')

# 绝对相位
im3 = axes[0, 2].imshow(absolute_phase, cmap='RdBu_r', origin='lower')
axes[0, 2].set_title('绝对相位 = 残差 + 参考')
plt.colorbar(im3, ax=axes[0, 2], label='rad')

# 理论相位
im4 = axes[1, 0].imshow(theory_phase, cmap='RdBu_r', origin='lower')
axes[1, 0].set_title('理论高斯光束相位')
plt.colorbar(im4, ax=axes[1, 0], label='rad')

# 相位差异
im5 = axes[1, 1].imshow(phase_diff, cmap='RdBu_r', origin='lower')
axes[1, 1].set_title('绝对相位 - 理论相位')
plt.colorbar(im5, ax=axes[1, 1], label='rad')

# 振幅
im6 = axes[1, 2].imshow(amplitude, cmap='hot', origin='lower')
axes[1, 2].set_title('振幅')
plt.colorbar(im6, ax=axes[1, 2])

plt.tight_layout()
plt.savefig('debug_phase_extraction_v2.png', dpi=150)
print(f"图表已保存到 debug_phase_extraction_v2.png")
