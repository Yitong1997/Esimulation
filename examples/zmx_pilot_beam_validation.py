"""
ZMX 文件端到端验证示例：仿真复振幅与 Pilot Beam 精度分析
...
def plot_comprehensive_analysis(
    ...
    for i, (metrics, data) in enumerate(zip(metrics_list, analysis_data_list)):
        # 仿真振幅
        im1 = axes[i, 0].imshow(
            data['sim_amplitude'] / np.max(data['sim_amplitude']),
...
# 由于 ZMX 文件可能不存在或格式不兼容，我们创建一个模拟的光学系统
# 这个系统模拟一个简单的激光扩束镜配置
...
def create_beam_expander_system() -> List[MockSurface]:
    """创建激光扩束镜光学系统
    
    系统配置：
    - M1: 凸面镜 (f=-100mm)，倾斜 45°，发散光束
    - M2: 凹面镜 (f=300mm)，倾斜 45°，准直光束
    - 放大倍率: M = -f2/f1 = 3x
    """
    surfaces = []
    ...
    # M1: 凸面镜，倾斜 45°
    tilt_x_rad = -np.pi / 4
    orientation_m1 = create_rotation_matrix_x(tilt_x_rad)
    
    m1 = MockSurface(
        index=0,
        surface_type='standard',
        vertex_position=np.array([0.0, 0.0, 100.0]),
        orientation=orientation_m1,
        radius=-200.0,  # 凸面镜，曲率中心在 -Z 方向
        conic=-1.0,     # 抛物面
        is_mirror=True,
        semi_aperture=15.0,
        material='mirror',
    ...
    # M2: 凹面镜，倾斜 45°
    # M2 位置：M1 反射后沿 -Y 方向传播 200mm
    orientation_m2 = create_rotation_matrix_x(tilt_x_rad)
    
    m2 = MockSurface(
        index=1,
        surface_type='standard',
        vertex_position=np.array([0.0, -200.0, 100.0]),
        orientation=orientation_m2,
        radius=600.0,   # 凹面镜，曲率中心在 +Z 方向
        conic=-1.0,     # 抛物面
        is_mirror=True,
        semi_aperture=50.0,
        material='mirror',
...
# 创建光学系统
optical_system = create_beam_expander_system()

print(f"光学系统配置:")
print(f"  表面数量: {len(optical_system)}")
for surface in optical_system:
    print(f"  - {surface.comment}: R={surface.radius:.1f}mm, 位置={surface.vertex_position}")
...
# 光源参数
wavelength_um = 10.64    # CO2 激光波长
w0_mm = 5.0              # 束腰半径
grid_size = 256          # 网格大小
physical_size_mm = 60.0  # 物理尺寸
...
if initial_metrics.phase_rms_error_waves < 0.001:
    print(f"  [OK] 初始波前精度满足要求 (< 0.001 waves)")
else:
    print(f"  [FAIL] 初始波前精度不满足要求")
...
plot_comprehensive_analysis(
    ...
    physical_size_mm=physical_size_mm,
    output_path="zmx_pilot_beam_accuracy.png",
)
...
for i, (metrics, data) in enumerate(zip(metrics_list, analysis_data_list)):
    ...
    # 第一行：振幅分析
    # 仿真振幅
    sim_amp_norm = data['sim_amplitude'] / np.max(data['sim_amplitude'])
    ...
    plt.tight_layout()
    fig.savefig(f"accuracy_detail_{metrics.surface_name}_{metrics.position}.png",
                dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f"[OK] 保存: accuracy_detail_{metrics.surface_name}_{metrics.position}.png")
...
生成的图像文件:
  • zmx_pilot_beam_accuracy.png - 综合分析图
...
# 评估结果
if metrics_list[0].phase_rms_error_waves < 0.001:
    print("  [OK] 初始波前精度满足严格要求 (< 0.001 waves)")
else:
    print("  [FAIL] 初始波前精度不满足严格要求")

if max(all_phase_rms) < 0.1:
    print("  [OK] 所有表面相位 RMS 误差 < 0.1 waves")
else:
    print("  [FAIL] 存在表面相位 RMS 误差 > 0.1 waves")

print("""
技术说明:
  • Pilot Beam 使用严格精确的高斯光束曲率公式
