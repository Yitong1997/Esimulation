...
class AsmAllocationPlan:
    ...
    def __post_init__(self) -> None:
        ...
        if self.estimated_peak_bytes != sum(value for _, value in self.components):
            raise ValueError("allocation total does not match its components")
...
def estimate_exact_peak_bytes(
    *,
    ...
    components = (
        ...
        ("czt_result", target_points * complex_bytes),
        ("retained_physical_outputs", 2 * target_points * complex_bytes),
        ("immutable_pointfield_copies", 2 * target_points * complex_bytes),
    )
    estimated = sum(value for _, value in components)
...
def propagate_helmholtz_pair(
    *,
    ...
    roi = np.asarray(primary_roi_mask)
    if roi.dtype != np.bool_ or roi.shape != (target_grid.ny, target_grid.nx) or not np.any(roi):
        raise ValueError("primary ROI mask must be a nonempty target-shaped Boolean array")
    ...
    del bl_values
    closure = _full_bl_closure(h_full, h_bl, roi)
    diagnostics = AsmDiagnostics(
